...
// .wrangler/tmp/bundle-jZ6QFN/checked-fetch.js
  ...
  try {
    ...
    await db.batch([
      ...
      db.prepare(`
            ...
            `).bind(crypto.randomUUID(), subscriberId, token2, expiresAt)
    ]);
    const confirmUrl = `${c.env.ORIGIN_BASE_URL}/confirm?token=${token2}`;
...
app.get("/api/confirm", async (c) => {
  ...
  try {
    const db = c.env.DB;
    const tokenRecord = await db.prepare("SELECT * FROM tokens WHERE token = ? AND type = 'confirm'").bind(token).first();
    if (!tokenRecord) {
      return c.json({ error: "Invalid or expired token" }, 400);
    }
    if (tokenRecord.expires_at < Math.floor(Date.now() / 1e3)) {
...
app.post("/api/unsubscribe", async (c) => {
  ...
  try {
    const db = c.env.DB;
    if (token) {
      const tokenRecord = await db.prepare("SELECT * FROM tokens WHERE token = ? AND type = 'unsubscribe'").bind(token).first();
      if (!tokenRecord) {
        return c.json({ error: "Invalid or expired token" }, 400);
      }
      await db.batch([
      ...
      const subscriber = await db.prepare("SELECT * FROM subscribers WHERE email = ?").bind(email).first();
      if (subscriber) {
        const token2 = crypto.randomUUID();
        const expiresAt = Math.floor(Date.now() / 1e3) + 60 * 60 * 24;
        await db.prepare("INSERT INTO tokens (id, subscriber_id, type, token, expires_at) VALUES (?, ?, 'unsubscribe', ?, ?)").bind(crypto.randomUUID(), subscriber.id, token2, expiresAt).run();
        const unsubscribeUrl = `${c.env.ORIGIN_BASE_URL}/unsubscribe?token=${token2}`;
        if (c.env.RESEND_API_KEY) {
...
app.post("/api/broadcast", async (c) => {
  ...
  try {
    ...
    const { results } = await db.prepare("SELECT email FROM subscribers WHERE status = 'confirmed'").all();
...
// .wrangler/tmp/bundle-jZ6QFN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jZ6QFN/middleware-loader.entry.ts
